...
export class Library implements Dirtable {
    ...
    private load(json: { [field: string]: any }, zip: Zip): void {
        ...
        let assets: { [id: string]: any } = json.assets;
        for (let id in assets) {
            ...
            let loader = LibraryAssetLoader.get(next.type);
            if (loader == null) {
                throw new Error(
    ...
    static read(path: string | Buffer, onSuccess: (library: Library) => void, onError: (error: Error) => void): void {
        ...
        zip.read(path, () => {
            ...
            let json = JSON.parse(zip.get('library.json').toString());
            ...
            try {
                let library = new Library(json.id, json.name);
...
export abstract class LibraryAsset {
    ...
    preUpdate(): void {
        try {
            ...
            console.error(
                "Failed to preUpdate() for LibraryAsset. (id: "
                + this.getId()
                + ", name: "
                + this.getName()
                + ")"
    ...
    update(): void {
        try {
            ...
            console.error(
                "Failed to update() for LibraryAsset. (id: "
                + this.getId()
                + ", name: "
                + this.getName()
                + ")"
    ...
    postUpdate(): void {
        try {
            ...
            console.error(
                "Failed to postUpdate() for LibraryAsset. (id: "
                + this.getId()
                + ", name: "
                + this.getName()
                + ")"
